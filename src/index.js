...
var redefine = createCommonjsModule((function(module) {
    ...
    (module.exports = function(O, key, value, options) {
        ...
        } else simple ? O[key] = value : setGlobal(key, value);
...
var createMethod = function(IS_INCLUDES) {
    return function($this, el, fromIndex) {
        ...
        if (IS_INCLUDES && el != el) {
            for (;length > index; ) if ((value = O[index++]) != value) return true;
...
var _export = function(options, source) {
    ...
    if (target = GLOBAL ? global_1 : STATIC ? global_1[TARGET] || setGlobal(TARGET, {}) : (global_1[TARGET] || {}).prototype) for (key in source) {
        ...
        if (!isForced_1(GLOBAL ? key : TARGET + (STATIC ? '.' : '#') + key, options.forced) && void 0 !== targetProperty) {
...
var arraySpeciesCreate = function(originalArray, length) {
    var C;
    isArray(originalArray) && ('function' != typeof (C = originalArray.constructor) || C !== Array && !isArray(C.prototype) ? isObject(C) && null === (C = C[SPECIES]) && (C = void 0) : C = void 0);
...
var arrayMethodHasSpeciesSupport = function(METHOD_NAME) {
    return engineV8Version >= 51 || !fails((function() {
        var array = [];
        (array.constructor = {})[SPECIES$1] = function() {
            return {
                foo: 1
            };
        };
...
_export({
    ...
    concat: function(arg) {
        ...
        for (i = -1, length = arguments.length; i < length; i++) if (isConcatSpreadable(E = -1 === i ? O : arguments[i])) {
            if (n + (len = toLength(E.length)) > MAX_SAFE_INTEGER) throw TypeError(MAXIMUM_ALLOWED_INDEX_EXCEEDED);
...
_export({
    ...
    slice: function(start, end) {
        ...
        if (isArray(O)) {
            'function' != typeof (Constructor = O.constructor) || Constructor !== Array && !isArray(Constructor.prototype) ? isObject(Constructor) && null === (Constructor = Constructor[SPECIES$2]) && (Constructor = void 0) : Constructor = void 0;
        ...
        for (n = 0; k < fin; k++, n++) k in O && createProperty(result, n, O[k]);
        result.length = n;
        return result;
    }
});
...
var FORCED$1 = nativeToFixed && ('0.000' !== 8e-5.toFixed(3) || '1' !== .9.toFixed(0) || '1.25' !== 1.255.toFixed(2) || '1000000000000000128' !== (0xde0b6b3a7640080).toFixed(0)) || !fails((function() {
    nativeToFixed.call({});
}));
...
_export({
    ...
    toFixed: function(fractionDigits) {
        ...
        var dataToString = function() {
            var index = 6;
            var s = '';
            for (;--index >= 0; ) if ('' !== s || 0 === index || 0 !== data[index]) {
        ...
        if (number > 1e-21) {
            z = (e = log(number * pow(2, 69, 1)) - 69) < 0 ? number * pow(2, -e, 1) : number / pow(2, e, 1);
            z *= 4503599627370496;
            if ((e = 52 - e) > 0) {
                multiply(0, z);
                j = fractDigits;
...
if (FORCED$2) {
    var RegExpWrapper = function(pattern, flags) {
        ...
        if (CORRECT_NEW) patternIsRegExp && !flagsAreUndefined && (pattern = pattern.source); else if (pattern instanceof RegExpWrapper) {
            flagsAreUndefined && (flags = regexpFlags.call(pattern));
            pattern = pattern.source;
        }
        UNSUPPORTED_Y$1 && (sticky = !!flags && flags.indexOf('y') > -1) && (flags = flags.replace(/y/g, ''));
    ...
    var index = 0;
    for (;keys$1.length > index; ) proxy(keys$1[index++]);
...
PATCH && (patchedExec = function(str) {
    ...
    } else re.lastIndex = 0; else UPDATES_LAST_INDEX_WRONG && match && (re.lastIndex = re.global ? match.index + match[0].length : lastIndex);
...
(NOT_GENERIC || INCORRECT_NAME) && redefine(RegExp.prototype, TO_STRING, (function() {
    ...
    var rf = R.flags;
    return '/' + p + '/' + String(void 0 === rf && R instanceof RegExp && !('flags' in RegExpPrototype$1) ? regexpFlags.call(R) : rf);
}), {
    unsafe: true
});
...
fixRegexpWellKnownSymbolLogic('match', 1, (function(MATCH, nativeMatch, maybeCallNative) {
    return [ function(regexp) {
        ...
        var n = 0;
        var result;
        for (;null !== (result = regexpExecAbstract(rx, S)); ) {
...
function multipleSolutionTest(model, variables, basicVariables, nonBasicVariables) {
    ...
    if (0 == primaryNonBasicVariables.length) return null;
    var pivotColumns = [];
...
var objectDefineProperties = descriptors ? Object.defineProperties : function(O, Properties) {
    ...
    var index = 0;
    var key;
    for (;length > index; ) objectDefineProperty.f(O, key = keys[index++], Properties[key]);
    return O;
};
...
var NullProtoObject = function() {
    ...
    for (;length--; ) delete NullProtoObject[PROTOTYPE][enumBugKeys[length]];
...
[].keys && ('next' in (arrayIterator = [].keys()) ? (PrototypeOfArrayIteratorPrototype = objectGetPrototypeOf(objectGetPrototypeOf(arrayIterator))) !== Object.prototype && (IteratorPrototype = PrototypeOfArrayIteratorPrototype) : BUGGY_SAFARI_ITERATORS = true);
...
var defineIterator = function(Iterable, NAME, IteratorConstructor, next, DEFAULT, IS_SET, FORCED) {
    ...
    if (DEFAULT) {
        ...
        if (FORCED) for (KEY in methods) (BUGGY_SAFARI_ITERATORS$1 || INCORRECT_VALUES_NAME || !(KEY in IterablePrototype)) && redefine(IterablePrototype, KEY, methods[KEY]); else _export({
            target: NAME,
            proto: true,
...
var toNumber = function(argument) {
    ...
    if ('string' == typeof it && it.length > 2) if (43 === (first = (it = trim$1(it)).charCodeAt(0)) || 45 === first) {
...
if (isForced_1(NUMBER, !NativeNumber(' 0o1') || !NativeNumber('0b1') || NativeNumber('+0x1'))) {
    ...
    for (var key, keys$2 = descriptors ? getOwnPropertyNames$1(NativeNumber) : 'MAX_VALUE,MIN_VALUE,NaN,NEGATIVE_INFINITY,POSITIVE_INFINITY,EPSILON,isFinite,isInteger,isNaN,isSafeInteger,MAX_SAFE_INTEGER,MIN_SAFE_INTEGER,parseFloat,parseInt,isInteger'.split(','), j = 0; keys$2.length > j; j++) has(NativeNumber, key = keys$2[j]) && !has(NumberWrapper, key) && defineProperty$3(NumberWrapper, key, getOwnPropertyDescriptor$2(NativeNumber, key));
...
var iterate_1 = createCommonjsModule((function(module) {
    ...
    (module.exports = function(iterable, fn, that, AS_ENTRIES, IS_ITERATOR) {
        ...
        next = iterator.next;
        for (;!(step = next.call(iterator)).done; ) if ('object' == typeof (result = callWithSafeIterationClosing(iterator, boundFunction, step.value, AS_ENTRIES)) && result && result instanceof Result) return result;
        return new Result(false);
    ...
    getConstructor: function(wrapper, CONSTRUCTOR_NAME, IS_MAP, ADDER) {
        ...
        redefineAll(C.prototype, {
            ...
            forEach: function(callbackfn) {
                ...
                var entry;
                for (;entry = entry ? entry.next : state.first; ) {
                    boundFunction(entry.value, entry.key, this);
                    for (;entry && entry.removed; ) entry = entry.previous;
                }
            },
            has: function(key) {
...
var $matchAll = function(string) {
    ...
    void 0 === (flagsValue = R.flags) && R instanceof RegExp && !('flags' in RegExpPrototype$2) && (flagsValue = regexpFlags.call(R));
...
_export({
    ...
    matchAll: function(regexp) {
        ...
        var S, matcher;
        if (null != regexp) {
            if (isRegexp(regexp) && !~String(requireObjectCoercible('flags' in RegExpPrototype$2 ? regexp.flags : regexpFlags.call(regexp))).indexOf('g')) throw TypeError('`.matchAll` does not allow non-global regexes');
            if (WORKS_WITH_NON_GLOBAL_REGEX) return nativeMatchAll.apply(O, arguments);
            void 0 === (matcher = regexp[MATCH_ALL]) && isPure && 'RegExp' == classofRaw(regexp) && (matcher = $matchAll);
            if (null != matcher) return aFunction$1(matcher).call(regexp, O);
        } else if (WORKS_WITH_NON_GLOBAL_REGEX) return nativeMatchAll.apply(O, arguments);
        S = String(O);
        return new RegExp(regexp, 'g')[MATCH_ALL](S);
    }
});
...
for (var COLLECTION_NAME$1 in domIterables) {
    ...
    if (CollectionPrototype$1) {
        ...
        if (domIterables[COLLECTION_NAME$1]) for (var METHOD_NAME in es_array_iterator) if (CollectionPrototype$1[METHOD_NAME] !== es_array_iterator[METHOD_NAME]) try {
...
function parseModel(objective, constraints) {
    ...
    var _objective$match2 = _slicedToArray(objective.match(objectiveRegex), 4), type = (_objective$match2[0], 
    _objective$match2[1]), objectiveVariable = _objective$match2[2], objectiveEquation = _objective$match2[3];
    type = type.toLowerCase();
    var _parseEquation2 = _slicedToArray(parseEquation(objectiveEquation), 2), objectiveCoeficients = _parseEquation2[0], objectiveVariables = _parseEquation2[1];
    constraints.forEach((function(d) {
        var _d$match2 = _slicedToArray(d.match(constraintRegex), 4), equation = (_d$match2[0], 
        ...
        var _parseEquation4 = _slicedToArray(parseEquation(equation), 2), constraintCoeficients = _parseEquation4[0], constraintVariables = _parseEquation4[1];
    ...
    var _buildTableau2 = _slicedToArray(buildTableau(modelVariables, modelCoeficients, modelConstraints, modelEqualities, objectiveVariable), 2);
    return [ _buildTableau2[0], _buildTableau2[1], type ];
}
...
function parseEquation(equation) {
    ...
    var coeficients = [];
    var variables = [];
    _toConsumableArray(equation.matchAll(elementRegex)).forEach((function(element) {
        var _element$0$match2 = _slicedToArray(element[0].match(coeficentRegex), 4), sign = (_element$0$match2[0], 
...
function buildTableau(variables, coeficients, constraints, equalities, objectiveVariable, type) {
    ...
    for (var i = 0; i < slackVariableCount; i++) uniqueVariables.push('s' + i);
    for (var _i = 0; _i < extraVariableCount; _i++) uniqueVariables.push('e' + _i);
    for (var _i2 = 0; _i2 < alternateVariableCount; _i2++) uniqueVariables.push('a' + _i2);
    ...
    equalities.reduce((function(a, b, i) {
...
function getPivot(model, variables, basicVariables, nonBasicVariables, type) {
    ...
    if (0 == (objectiveValues = 'max' == type ? objectiveValues.filter((function(d) {
        return trim(d) < 0;
    })) : objectiveValues.filter((function(d) {
        return trim(d) > 0;
    }))).length) {
        return null == multipleSolutionTest(model, variables, basicVariables, nonBasicVariables) ? 'solved' : 'multiple solutions';
    }
    ...
    switch ((pivotRows = model.reduce((function(a, b, i) {
    ...
    return {
        row: pivotRow = null == pivotRow ? pivotRows[0] : pivotRow,
        column: pivotColumn
    };
}
...
_export({
    ...
    splice: function(start, deleteCount) {
        ...
        for (k = 0; k < actualDeleteCount; k++) (from = actualStart + k) in O && createProperty(A, k, O[from]);
...
function buildPhaseOneTableau(model, variables) {
    ...
    phaseOneTableau.forEach((function(row) {
        for (var index = 0; index < row.length; index++) if (testVariable(variables[index], [ 'a' ]) && 1 == row[index]) {
...
function cleanPhaseOneTableau(model, objective, variables, basicVariables, nonBasicVariables) {
    ...
    if (trim(model[lastRow][lastColumn]) > 0) return [ model, 'infeasible' ];
    ...
    nonBasicVariables.reduce((function(a, b, i) {
        return testVariable(b, [ 'a' ]) ? a.concat(i) : a;
    }), []).reverse().forEach((function(index) {
        nonBasicVariables.splice(index, 1);
    }));
    return [ model = reBaseModel(model, variables, basicVariables), '' ];
}
...
function buildSolution(model, basicVariables, nonBasicVariables, result) {
    var solution = [];
    var lastColumn = model[0].length - 1;
    for (var i = 0; i < basicVariables.length; i++) solution.push([ basicVariables[i], trim(model[i][lastColumn]) ]);
    return {
        solution: solution,
        result: result
    };
...
function phaseTwo(model, variables, basicVariables, nonBasicVariables, type) {
    var pivot;
    for (;;) {
        switch (pivot = getPivot(model, variables, basicVariables, nonBasicVariables, type)) {
          case 'solved':
...
function simplex(objective, constraints) {
    var _parseModel2 = _slicedToArray(parseModel(objective, constraints), 3), model = _parseModel2[0], variables = _parseModel2[1], type = _parseModel2[2];
    if (0 != model.length) {
        var tableau;
        var result;
        model.forEach((function(row) {
        ...
        if (variables.some((function(variable) {
            ...
            var _phaseTwo2 = _slicedToArray(phaseTwo(tableau = buildPhaseOneTableau(model, variables), variables, basicVariables, nonBasicVariables, 'min'), 2);
            tableau = _phaseTwo2[0];
            if ('unbounded' == (result = _phaseTwo2[1])) return buildSolution(tableau, basicVariables, nonBasicVariables, result);
            var _cleanPhaseOneTableau2 = _slicedToArray(cleanPhaseOneTableau(tableau, originalObjective, variables, basicVariables, nonBasicVariables), 2);
            tableau = _cleanPhaseOneTableau2[0];
            if ('infeasible' == (result = _cleanPhaseOneTableau2[1])) return buildSolution(tableau, basicVariables, nonBasicVariables, result);
            var _phaseTwo4 = _slicedToArray(phaseTwo(tableau, variables, basicVariables, nonBasicVariables, type), 2);
            tableau = _phaseTwo4[0];
            result = _phaseTwo4[1];
        } else {
            var _phaseTwo6 = _slicedToArray(phaseTwo(model, variables, basicVariables, nonBasicVariables, type), 2);
